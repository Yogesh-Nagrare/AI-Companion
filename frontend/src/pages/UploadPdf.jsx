...
const STEPS = [
  ...
  { id: 'analyzing',  label: 'AI analysis (Gemini)'},
];
...
export default function UploadPdf() {
  ...
  const [pdfPreview, setPdfPreview] = useState(null); // Base64 preview
  const fileInputRef = useRef(null);

  // Reset slice on mount/unmount
  useEffect(() => {
    dispatch(resetUpload());
    return () => dispatch(resetUpload());
  }, [dispatch]);

  // Redirect when analysis is done
  useEffect(() => {
    if (uploadStep === 'done' && currentAnalysis?.reportId) {
      const timer = setTimeout(() => navigate(`/analysis/${currentAnalysis.reportId}`), 900);
  ...
  // ── Generate PDF thumbnail preview ──
  const generatePdfPreview = async (file) => {
    try {
      const arrayBuffer = await file.arrayBuffer();
      const pdf = await getDocument({ data: await file.arrayBuffer() }).promise;      const page = await pdf.getPage(1);
  ...
  const handleFileSelect = (selected) => {
    ...
    // Auto-fill report name from filename only if user hasn't typed one yet
    setReportName((prev) =>
  ...
  const handleAnalyze = async () => {
    ...
    const signatureData = sigResult.payload;
    ...
    const cloudinaryData = uploadResult.payload;
      ...
      <main className="relative z-10 max-w-xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 text-sm mt-3 leading-relaxed max-w-sm mx-auto">
            Upload any health PDF and get structured AI insights instantly.
          </p>
        </div>
        ...
        <div className="bg-[#0a0f12]/80 border border-white/5 rounded-[32px] backdrop-blur-xl p-8 space-y-6">
              ...
              {file ? (
                ...
                >
                  <div className="p-4 bg-teal-500/10 rounded-2xl border border-teal-500/20 flex items-center justify-center">
                    {pdfPreview ? (
                      <img src={pdfPreview} alt="PDF preview" className="w-16 h-20 object-contain rounded-md shadow-md" />
                    ) : (
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. HbA1c Test — January 2025"
          ...
          <AnimatePresence>
            {uploadStep === 'done' && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
            ...
            {error && uploadStep !== 'done' && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-start gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
                <AlertCircle size={16} className="text-red-400 flex-shrink-0 mt-0.5" />
                <div className="flex-1">
                  <span className="text-xs text-red-300">{error}</span>
                  <button
                    onClick={() => { dispatch(resetUpload()); setLocalError(''); setPdfPreview(null); }}
                    className="block mt-2 text-[10px] text-red-400 hover:text-red-300 font-bold uppercase tracking-widest transition-colors"
                  >
                    ↺ Try again
                  </button>
                </div>
          ...
          <button
            ...
            disabled={!file || isProcessing || uploadStep === 'done'}
          ...
          >
            {isProcessing ? <><Loader2 size={15} className="animate-spin" /> Processing...</> : <><Sparkles size={15} /> Analyze with Gemini</>}
          </button>

          <p className="text-center text-[10px] text-slate-600 leading-relaxed">
            PDF is encrypted in transit · Analysis takes 10–25 seconds
          </p>
        </div>
      </main>
    </div>
